...
function detectDependencies(config) {
  ...
  config.set('global-dependencies-sorted', filterExcludedDependencies(
    config.get('detectable-file-types').reduce(function(acc, fileType) {
      if (!acc[fileType]) {
...
function gatherInfo(config) {
  ...
  return function(version, component) {
    ...
    if (!componentConfigFile) {
      var error;
      if (config.get('bower.json').fakeBower) {
        error = new Error(`${component}:${version} is not installed. Try running \`yarn\` or remove the component from your package.json file.`);
      } else {
        error = new Error(`${component}:${version} is not installed. Try running \`bower install\` or remove the component from your bower.json file.`);
      }
    ...
    if (componentConfigFile.dependencies) {
      ...
      if (config.get('verbose')) {
        console.log(`${component}:${version} has following dependencies`);
